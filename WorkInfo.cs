...
namespace DAL
{
    public class WorkInfo
    {
        ...
        public int Add(Model.WorkInfo workInfo)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "insert into workinfo(workId,workName,workPictuer,uoloadTime,workIntroduction,workVerify,userName,activityId,recommend,recommendTime)"+"values(@workId,@workName,@workPicture,@uploadTime,@workIntroduction,@woorkVerify,@userName,@activityId,@recommend,@recommendTime);";
                sql += "SELECT @@IDENTITY";
